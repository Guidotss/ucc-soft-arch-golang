...
func CoursesRoutes(g *gin.Engine, controller *courses.CourseController) {
	...
	g.POST("/courses/create", middlewareAdmin.AdminAuthMiddleware(), controller.Create)
	g.GET("/courses", controller.GetAll)
	g.PUT("/courses/update", middlewareCourse.CheckCourseId(), middlewareAdmin.AdminAuthMiddleware(), controller.UpdateCourse)
